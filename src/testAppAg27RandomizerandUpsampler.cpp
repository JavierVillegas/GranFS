...
void testApp::setup(){
    ...
    // rotate all the input array 90 degrees
    
    if(doRotation){
    vector <cv::Mat> RotatedFrames;
        cv::Mat RotIm;
        RotIm.create(TheFramesInput[0].rows, Nlast, CV_8UC3);
         unsigned char *output = (unsigned char*)(RotIm.data);
        for (int x = BlockDims.x-1; x >= 0; x--){
            for (int t = 0; t< BlockDims.z; t++) {
                unsigned char *input = (unsigned char*)(TheFramesInput[t].data);
                
                   for (int y = 0; y < BlockDims.y; y ++) {
                    output[3*RotIm.cols * (y) + 3*(t) ] =  input[(int)(3*BlockDims.x * (y) + 3*(x)) ] ;
                    output[3*RotIm.cols * (y) + 3*(t) + 1] =  input[(int)(3*BlockDims.x * (y) + 3*(x)) +1] ;
                    output[3*RotIm.cols * (y) + 3*(t) + 2] =  input[(int)(3*BlockDims.x * (y) + 3*(x)) +2 ] ;
                    }
            }
            RotatedFrames.push_back(RotIm.clone());
        }
    
    
        TheFramesInput = RotatedFrames;
        BlockDims.z = BlockDims.x;
        BlockDims.x = Nlast;
        RotatedFrames.clear();
    }
